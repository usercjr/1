...
{{"analysis":{{"A":"<一句话推理依据，引用证据编号>","B":"...","C":"...","D":"..."}},"answer":"<按字母升序排列的所有正确选项，如 AC>","reasoning":"<3句以内推理摘要：文档定位依据→关键证据（含具体数字/条款）→结论>"}}

判断规则：
- 独立判断每个选项是否被证据明确支持；证据未提及的选项视为错误，不要纳入答案。
- 漏选与错选同样计为全错：证据（含任何一条证据片段）明确支持的选项必须全部选入，一个都不能漏；证据不支持的一个都不能选。
- 数值/日期类选项：先在证据中找到对应数字再比对，数字与科目/口径都相符才算支持。
- 口径围栏：数字相符但科目/口径不符（如 营业收入≠新签合同额、分部数据≠全公司、常规分红≠含特别分红的总额）→ 该选项不成立；严禁"此处应指"式的科目转换解释。
- 尾巴替换围栏：选项前半句与证据相符，但结论/对象被偷换的（如证据"支撑了保费的快速增长"被选项改成"支撑了管理体系"、证据主体是甲公司被选项换成乙公司）→ 该选项不成立。须将选项的完整断言逐段与证据比对，不能只核对前半句或数字。
- 证据未给出比较基期数据时，不得推断同比趋势（"可以推断出增长/下降"都是错误做法，应视为证据不足）。
...
请按下列 JSON 格式输出，不要任何额外文字、不要 markdown 代码块、不要 ```:
{{"analysis":{{"A":"<一句话依据>","B":"...","C":"...","D":"..."}},"answer":"<唯一正确选项字母 A/B/C/D>","reasoning":"<3句以内推理摘要：文档定位依据→关键证据→结论>"}}
...
请按下列 JSON 格式输出，不要任何额外文字、不要 markdown 代码块、不要 ```:
{{"analysis":{{"A":"<陈述与证据一致性分析>","B":"<反方依据>"}},"answer":"<A 或 B>","reasoning":"<3句以内推理摘要：文档定位依据→关键证据→结论>"}}
